...
export class OllamaHandler extends BaseProvider implements SingleCompletionHandler {
	...
	override async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
		console.info(
			`createMessage():\r\n${messages.map((m) => `___${m.role}___: ${Array.isArray(m.content) ? m.content.map((c) => JSON.stringify(c)).join("\r\n") : m.content}`).join("\r\n")}`,
		)
				...
				tools: [
					...
					},
					{
						...
						function: {
							name: "search_files",
							description:
								"Request to perform a regex search across files in a specified directory, providing context-rich results. This tool searches for patterns or specific content across multiple files, displaying each match with encapsulating context.  regex parameter is required",
							parameters: {
								type: "object",
								properties: {
									path: { type: "string" },
...
When applying the diffs, be extra careful to remember to change any closing brackets or other syntax that may be affected by the diff farther down in the file`,
			...
			for (const choice of stream.choices) {
				if (choice.message.content) {
					...
					for (const tool of choice.message.tool_calls) {
						console.info(`TOOL_CALL: ${tool.function.name}(${tool.function.arguments})`)
			...
			for (const chunk of matcher.update(`${error}`)) {
				yield chunk
			}
			for (const chunk of matcher.final()) {
				yield chunk
